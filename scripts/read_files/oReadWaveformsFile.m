...
%[times,waveforms] = nptLoadingEngine(filename,r2g,ru)
% 	Read in Waveform file containing spike times and
%   waveforms for MCLust Spike sorting Program
%	[t,wv] = nptLoadingEngine(FILENAME,r2g,ru) opens FILENAME and 
...
switch(ru)
    ...
    case 2  %records list
        fprintf('Calling Records List  !!!When is this used???')
...
if ~isempty(wv)
    if cha~=4
        z=zeros((4-cha)*32,size(wv,2));
        wv = [wv;z];
    end
    wv = reshape(wv,[32 4 size(wv,2)]);
    wv = permute(wv,[3 2 1]);
end
