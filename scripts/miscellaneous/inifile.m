function [readsett,result] = inifile(fileName,operation,keys,style)

%INIFILE Creates, reads, or writes data from/to a standard ini (ascii) file

%   [readsett,result] = INIFILE(fileName,operation,keys,style)

%   Creates, reads, or writes data from/to a standard ini (ascii) file. Such

%   a file is organized into sections ([section name]),

%   subsections (enclosed by {subsection name}), and keys (key=value).

%   Empty lines and lines with first non-empty character being ; (comment

%   lines).

%

%   - fileName:     ini file name (a string)
...
%   - keys:         cell array of STRINGS; either 5 columns or 3 columns. 

%                   Each row has the same number of columns. The columns are:

%                   'section':      section name string (the root is considered if empty)

%                   'subsection':   subsection name string (the root is considered if empty)

%                   'key':          name of the field to write/read from (given as a string).

%                   value:          (optional) STRING or NUMERIC value (scalar or matrix) to be written to the
...
%                   defaultValue:   (optional) string or numeric value (scalar or matrix) that is returned when the key is not found

%                                   when reading ('read' operation).

%                                   If not given and the key is not found, an empty value is returned.
...
%   - readsett:     (optional) a cell array of string and/or numeric values - read setting in the case of the 'read' operation.

%                   If the keys are not found, the default values are returned (if given in the 5-th column).

%   - result:       (optional) a cell array of strings - one for each key read; Empty

%                   if OK, error/warning string if conversion fails. In

%                   both cases an empty string is returned in readSett{i} for the i-th key.

%

%   EXAMPLE:

%   Suppose we want a new ini file, test1.ini with 4 fields, including a 5x5 matrix (see below).

%   We can write them into the file using:

%

%   x = rand(5);    % data as a matrix
...
%   exist, a default value will be returned (if the 5-th column is given for all the rows as below).
...
%   '1 2 3;4 5 6' or '1,2,3;4,5,6' (an example). Comment lines starts with

%   ; as the first non-empty character but comments can not exist as a tail to

%   a standard, non-comment line as ; is also used as a row delimiter for matrices.
...
%   Copyright (c) 2003-2004 by Primoz Cermelj
...
% - NEW: Comment lines are detected and thus ignored; comment lines are lines with first non-empty character being ;

% - NEW: Lines not belonging to any of the recognized types (key, section, comment,...) raise an error.

%

% [v.1.3.0] 21.04.2004

% - NEW: Numeric matrices can be read/written
...
% - NEW: Conversion ability (integers, doubles, and strings) added for read and write operations
...
%        read/written keys are considered with respect to the older version.
...
if nargin == 0
    ...
    disp('Copyright (c) 2003-2004 by Primoz Cermelj');

    disp('This is FREE SOFTWARE');

    disp('Type "help inifile" (without ")');

    disp('to get more help on its usage');

    return

elseif nargin < 2

    error('inifile:argNumber','Not enough input arguments');
...
if (strcmpi(operation,'read')) | (strcmpi(operation,'deletekeys'))
    ...
    if ~exist(fileName)
        ...
        error('inifile:fileDoesNotExist',['File ' fileName ' does not exist.']);
    ...
    if n > 5
        ...
        error('inifile:notEnoughColumnsInKeys','Keys argument has too many columns');
    ...
    for ii=1:m
        ...
        if isempty(keys(ii,3)) | ~ischar(keys{ii,3})
            ...
            error('inifile:nonCharKey','Empty or non-char keys are not allowed.');
...
elseif (~strcmpi(operation,'new'))
    ...
    error('inifile:unknownOperation',['Unknown inifile operation: ''' operation '''']);
    ...
    for ii=1:m
        ...
        for jj=1:3
            ...
            if ~ischar(keys{ii,jj})
                ...
                error('inifile:nonCharFields','All cells from the first 3 columns must be given as strings, even the empty ones.');
...
if nargin < 4 || isempty(style)
    ...
    if ~(strcmpi(style,'plain') | strcmpi(style,'tabbed')) | ~ischar(style)
        ...
        error('inifile:invalidStyle','Unsupported style given or style not given as a string');
...
if strcmpi(operation,'new')
    ...
    if fh == -1
        ...
        error('inifile:fileNotAccessible',['File: ''' fileName ''' can not be (re)created']);
    ...
    for ii=1:m  % go through all the keys
        ...
        if ~isempty(conversionOp{ii}) & ~strcmpi(conversionOp{ii},'s')
            ...
            if strcmpi(conversionOp{ii},'i') | strcmpi(conversionOp{ii},'d')
                ...
                error('inifile:unknownConversionChar',['Invalid conversion char given: ' conversionOp{ii}]);
...
elseif (strcmpi(operation,'write'))
    ...
    if m < 1
        ...
        error('inifile:noKeysGiven','At least one key is needed when writing keys');
...
elseif (strcmpi(operation,'deletekeys'))
    ...
    error('inifile:unknownOperation','Unknown operation for INIFILE.');
...
if fh == -1
    ...
    error('inifile:fileNotAccessible',['File: ''' fileName ''' does not exist or can not be opened.']);
...
try
    ...
    error('inifile:keysParsingError',['Error parsing the file for keys: ' fileName ': ' lasterr]);
...
if n < 4
    ...
    error('inifile:invalidKeysFormat','Keys to be written are given in an invalid format.');
...
try
    ...
    error('inifile:writingFailed',['Error writing keys to file: ''' fileName ''' : ' lasterr]);
...
if n < 3
    ...
    error('inifile:invalidKeysFormat','Keys to be deleted are given in an invalid format.');
...
try
    ...
    error('inifile:deletingFailed',['Error deleting keys from file: ''' fileName ''' : ' lasterr]);
...
% Example: given x = [1 2;3 4] returns y = '1,2;3;4'
...
for ii=1:m
    ...
    str = [str ';' num2str(x(ii,1),'%15.6g')];

    for jj=2:n

        str = [str ',' num2str(x(ii,jj),'%15.6g')];
