...
%   columns of MAT. If MAT is a matrix with two columns, the angle between 
...
%   rows of MAT and computing the percentage of surrogates with
...
vmags = sqrt(sum(mat.^2));
% create matrix of magnitudes to do division
magmat = repmat(vmags,mrows,1);
...
if(mcols==2)
    ...
    mat1 = mat(:,1)';
    warning off MATLAB:divideByZero
	angle = rad2deg(acos((mat1 * mat(:,2))/vmprod));
    warning on MATLAB:divideByZero
    if(nargout>2)
